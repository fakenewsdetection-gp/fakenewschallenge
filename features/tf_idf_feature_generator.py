...
class TfidfFeatureGenerator():
    ...
    def process_train(self, train, test, max_num_words=5000):
        """
    ...
    def process_test(self, test, bow_vectorizer, tfreq_vectorizer, tfidf_vectorizer):
        """
        Process test set
        ...
        self.save(test_data, header='test')
        return test_data

    def load(self, header='train'):
        filename = '.'.join([header, self.name, 'pickle'])
        return pd.read_pickle(filename)

    def save(self, df, header='train'):
        filename = '.'.join([header, self.name, 'pickle'])
